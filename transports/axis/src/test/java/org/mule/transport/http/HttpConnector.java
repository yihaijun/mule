/*
 * $Id: HttpConnector.java 24749 2012-08-15 22:13:17Z svacas $
 * --------------------------------------------------------------------------------------
...
public class HttpConnector extends TcpConnector
{
    ...
    protected void doInitialise() throws InitialisationException
    {
        ...
        if(logger.isDebugEnabled()){
                /*
        asasip-debug.log.2012-09-10.log:2012-09-10 18:27:48,725 
        getServiceDescriptor()=org.mule.transport.service.DefaultTransportServiceDescriptor@26da2af3
        dispatchers.getMaxTotal()=8000,dispatchers.getNumActive()=0/400,dispatchers.getNumIdle()=0/400,
        clientConnectionManager=null [org.mule.transport.http.HttpConnector] DEBUG [WrapperListener_start_runner]             
                 */
            logger.debug("name="+getName()+",getServiceDescriptor()="+getServiceDescriptor()+",dispatchers.getMaxTotal()="+dispatchers.getMaxTotal()+",dispatchers.getNumActive()="+dispatchers.getNumActive()+"/"+dispatchers.getMaxActive()+",dispatchers.getNumIdle()="+dispatchers.getNumIdle()+"/"+dispatchers.getMaxIdle()+",clientConnectionManager="+clientConnectionManager);
        }
        if(dispatchers.getMaxActive()>200){
            logger.warn("dispatchers.getMaxActive()="+dispatchers.getMaxActive()+"> 200,now setMaxActive(200).(getServiceDescriptor()="+getServiceDescriptor()+",name="+getName()+")");
            dispatchers.setMaxActive(200);
        }
        if(dispatchers.getMaxIdle() > 50){
            logger.warn("dispatchers.getMaxIdle()="+dispatchers.getMaxIdle()+"> 50,now setMaxIdle(50).(getServiceDescriptor()="+getServiceDescriptor()+",name="+getName()+")");
            dispatchers.setMaxIdle(50);
        }

        if(dispatchers.getMaxTotal() > 2000){
            logger.warn("dispatchers.getMaxTotal()="+dispatchers.getMaxTotal()+"> 2000,now setMaxTotal(2000).(getServiceDescriptor()="+getServiceDescriptor()+",name="+getName()+")");
            dispatchers.setMaxTotal(2000);
        }
        if(dispatchers.getNumActive()>10){
            logger.warn("dispatchers.getNumActive()="+dispatchers.getNumActive()+"> 10.(getServiceDescriptor()="+getServiceDescriptor()+",name="+getName()+")");
        }
        if (clientConnectionManager == null)
        {
            ...
            if(dispatchers.getMaxTotal()<=400){
                params.setDefaultMaxConnectionsPerHost(dispatchers.getMaxTotal());
            }else{                
                logger.warn("dispatchers.getMaxTotal()="+dispatchers.getMaxTotal()+"> 400,now setDefaultMaxConnectionsPerHost(150).(getServiceDescriptor()="+getServiceDescriptor()+",name="+getName()+")");
